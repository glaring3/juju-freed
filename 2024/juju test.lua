...
function target_info:set_gun(gun)
	...
	if gun then
		...
		target_info.drawings[2].Size = vector2new(240,118)
		target_info.drawings[3].Size = vector2new(240,116)
		target_info.drawings[4].Size = vector2new(240,112)
		target_info.drawings[5].Size = vector2new(240,110)
	else
		target_info.drawings[1].Size = vector2new(240,100)
		target_info.drawings[2].Size = vector2new(240,98)
		target_info.drawings[3].Size = vector2new(240,96)
		target_info.drawings[4].Size = vector2new(240,92)
		target_info.drawings[5].Size = vector2new(240,90)
	end
end
	...
	utility.newConnection(character.ChildAdded, function(object)
		...
		if object.ClassName == "Tool" then
			...
			if ammo then
				...
				if shootbbgui then
					particle_connection = utility.newConnection(shootbbgui.Shoot:GetPropertyChangedSignal("Visible"), LPH_JIT(function()
						local skin = skins["equipped"][object.Name]
				...
				activate_connection = utility.newConnection(object.Activated, LPH_JIT(function()
				...
				ammo_connection = utility.newConnection(object.Ammo:GetPropertyChangedSignal("Value"), LPH_JIT(function()
...
local legit = window:getTab(3)
	local aimbot_section = legit:newSection({name = "Aimbot settings", is_changeable = false, scale = 0.7})
		...
		aimbot_section:newElement({name = "Anti aim viewer", types = {toggle = {flag = "anti_aim_viewer"}}})
	...
	local gun_settings = legit:newSection({name = "Gun settings", is_changeable = false, x = 0.5, scale = 1})
		...
		for gun, _ in gun_types do
			if gun ~= "Default" then
				...
				utility.newConnection(_["settings"][5].onSliderChange, function(value)
					if lplr_data["gun"] == gun and flags[gun.."_override_general"] then
...
local visuals = window:getTab(4)
	...
	local hud_section = visuals:newSection({name = "Hud", is_changeable = true, x = 0.5, y = 0.3, scale = 0.35})
		...
		menu_references["target_font"] = hud_section:newElement({name = "Font", types = {dropdown = {options = {"1", "2", "3"}, no_none = true, flag = "target_font", default = {"2"}}}})
	...
	local other_section = visuals:newSection({name = "Other", is_changeable = true, x = 0.5, y = 0.65, scale = 0.35})
		...
		menu_references["damage_number_font"] = other_section:newElement({name = "Font", types = {dropdown = {flag = "damage_number_font", options = {"1", "2", "3"}, default = {"2"}, no_none = true}}})
			...
			if flags["aim_assist"] and flags["aim_assist_keybind"]["active"] then
				...
				for _, check in flags["aim_assist_disable_when"] do
					if check == "Typing" and lplr_data["typing"] then
						stop = true
						break
					elseif check == "Not holding gun" and not lplr_data["gun"] then
						stop = true
						break
				...
				if not stop then
					...
					if flags["aim_assist_style"][1] == "Mouse" then
						...
						if aimbot.do_tp then
							...
							mousemoverel(distance.X * getValue(tws, ((101-flags[gun.."_aim_assist_horizontal_smoothness"])/100), ease_style, direction), distance.Y * getValue(tws, (101-flags[gun.."_aim_assist_vertical_smoothness"])/100, ease_style, direction))	
						end
					else
						if aimbot.do_tp then
							camera.CFrame = cframenew(camera.CFrame, position)
							aimbot.do_tp = false
						else
							camera.CFrame = camera.CFrame:Lerp(cframenew(camera.CFrame.p, position), getValue(tws, ((101-flags[gun.."_aim_assist_horizontal_smoothness"])/100), Enum.EasingStyle[flags[gun.."_smoothing_style"][1]], Enum.EasingDirection[flags[gun.."_smoothing_direction"][1]]))
						end
					end
...
local renderDamageNumber = LPH_NO_VIRTUALIZE(function(player, damage)
	...
	local text = utility.newDrawing("Text", {
		...
		Font = tonumber(flags["damage_number_font"][1]),
...
utility.newConnection(menu_references["auto_stomping"].onToggleChange, function(bool)
	local player = plrs:FindFirstChild(menu_references["players_box"]["selected_option"])
