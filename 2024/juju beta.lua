...
local mathrandom = math.random
	...
	local rpg_icon = game:HttpGet("https://raw.githubusercontent.com/jujudotlol/jujudotlol.github.io/main/rpg_icon.png")
	local tool_images = {
...
skins["8-BIT"] = {
	...
	ShootSounds = {
		...
		["double-barrelsg"] = "rbxassetid://13326578563"
	},
...
for _, skin in require(reps.SkinModules) do
    ...
    for skin_name, info in skin do
		...
		if info["Rarity"] == "Exclusive" and info["BorderColor"] and not skins[skin_name] then
			skins[skin_name] = {
				Color = ColorSequence.new{ColorSequenceKeypoint.new(0,info["BorderColor"][1]), ColorSequenceKeypoint.new(1,info["BorderColor"][1])}
			}
		end
    end
end

local applySkin = LPH_JIT(function(tool, skin, knife)
    ...
    if typeof(texture_id) == "string" then
        ...
        local new_default = reps.SkinModules[knife and "Knives" or "Meshes"]:FindFirstChild(skin)
		
		if not new_default then
			for _, child in default:GetChildren() do
				child:Destroy()
			end
		...
		new_default = knife and new_default:Clone() or new_default[texture_id.Name]:Clone()
...
local purchaseItem = LPH_JIT_MAX(function(name)
	...
	local bought = false
	local ping = (flags["purchase_delay"] ~= 0 and flags["purchase_delay"]/1000 + lplr_data["ping"]/1000) or lplr_data["ping"]/1000
	local old_cf = hrp.CFrame

	for _, tool in lplr.Backpack:GetChildren() do
		if utility.removeBrackets(tool.Name) == name then
			bought = true
			break
		end
	end
	...
	if ammo and flags["purchase_ammo"] > 0 then
		if not bought then
			lplr_data["force_cframe"] = cframenew(shop[1].Position-vector3new(0,5.5,0))
			_wait(ping*3)
			fireclickdetector(shop[2])
			_wait(ping)
		end
		lplr_data["force_cframe"] = cframenew(ammo[1].Position-vector3new(0,5.5,0))
		_wait(ping*3)
...
utility.newRPGIndicator = LPH_NO_VIRTUALIZE(function(rocket, pos)
	local indicator = {
		utility.newDrawing("Image", {
...
local shootsounds = {SSG08 = "rbxassetid://2476571739", SCAR20 = "rbxassetid://2476571739", AWP = "rbxassetid://1112948895"}
	...
	for player, info in player_data do
		...
		for _, check in checks do
			...
			elseif check == "Behind wall" then
				...
				raycast_params.FilterDescendantsInstances = {character, char, workspace.Ignored, workspace.Vehicles}
...
local playerCharacterAdded = LPH_NO_VIRTUALIZE(function(character)
	...
	local armor = body_effects:WaitForChild("Armor", 60)
	local grabbed = body_effects:WaitForChild("Grabbed", 60)
	local knocked = body_effects:WaitForChild("K.O", 60)
	...
	utility.newConnection(humanoid:GetPropertyChangedSignal("Health"), function()
		...
		utility.doCustomTween(data, "health", health)
...
local hud_styles = {
	["Default"] = LPH_JIT(function()
		if not main_gui then
			return end
	
		local bar = main_gui:FindFirstChild("Bar")
		...
		for _, object in bar:GetChildren() do
			...
			local corner = object.bar:FindFirstChildOfClass("UICorner")
			if corner then
				corner:Destroy()
			end
			local line = object:FindFirstChild("line")
			if line then
				line:Destroy()
			end
			local shadow = object:FindFirstChild("shadow")
			if shadow then
				shadow:Destroy()
			end
			object.BackgroundColor3 = Color3.fromRGB(58,58,58)
			object.Picture.Visible = true
			object.BackgroundTransparency = 0
			object.bar.BackgroundTransparency = object.Name == "Armor" and 0.3 or 0
			if object.Name == "Armor" then
				object.firebar.BackgroundTransparency = 0
				local corner = object.firebar:FindFirstChildOfClass("UICorner")
				if corner then
					corner:Destroy()
				end
			end
			object.TextLabel.TextXAlignment = object.Name == "Energy" and Enum.TextXAlignment.Right or object.Name == "Armor" and Enum.TextXAlignment.Left or Enum.TextXAlignment.Center
			object.TextLabel.TextSize = 18
			object.TextLabel.Font = Enum.Font.SourceSansBold

			object.TextLabel.Text = string.upper(sub(object.TextLabel.Text, 1, 1))..sub(object.TextLabel.Text, 2, #object.TextLabel.Text)
		end
	end),
	["Minimalistic"] = LPH_JIT(function()
		if not main_gui then
			return end
		...
		for _, object in bar:GetChildren() do
			...
			object.TextLabel.TextXAlignment = Enum.TextXAlignment.Center
			object.TextLabel.TextSize = 14
			object.TextLabel.Text = lower(object.TextLabel.Text)
			object.TextLabel.Font = Enum.Font.GothamBold
		end
	end),
	["Simple dark"] = LPH_JIT(function()
		if not main_gui then
			return end
		...
		for _, object in bar:GetChildren() do
			...
			object.TextLabel.TextSize = 14
			object.TextLabel.Text = lower(object.TextLabel.Text)
			object.TextLabel.Font = Enum.Font.ArialBold
		end	
	end),
	["Gamesense"] = LPH_JIT(function()
		if not main_gui then
			return end
...
local beam_styles = {
	...
	["Flash"] = newObject("Beam", {
		LightEmission = 1;
		LightInfluence = 1;
		Texture = [[rbxassetid://12781825175]];
		TextureLength = 0.10000000149011612;
		TextureSpeed = 2;
		Transparency = NumberSequence.new{NumberSequenceKeypoint.new(0,0,0),NumberSequenceKeypoint.new(1,1,0)};
		Width0 = 0.30000001192092896;
		Width1 = 0.30000001192092896;
		FaceCamera = true;
	}),
	["Flow"] = newObject("Beam", {
		Color = ColorSequence.new{ColorSequenceKeypoint.new(0,Color3.new(0,0.333333,0)),ColorSequenceKeypoint.new(1,Color3.new(0,0.333333,0))};
		LightEmission = 1;
		LightInfluence = 1;
		Texture = [[rbxassetid://9632166076]];
		TextureLength = 0.625;
		TextureSpeed = -1;
		FaceCamera = true;
		Transparency = NumberSequence.new{NumberSequenceKeypoint.new(0,0,0),NumberSequenceKeypoint.new(1,1,0)};
	...
	["Electric"] = newObject("Beam", {
		Color = ColorSequence.new{ColorSequenceKeypoint.new(0,Color3.new(0,0.666667,1)),ColorSequenceKeypoint.new(1,Color3.new(0,0.666667,1))};
		LightEmission = 1;
		LightInfluence = 1;
		Texture = [[http://www.roblox.com/asset/?id=1134824633]];
		TextureSpeed = 2;
		FaceCamera = true;
		Transparency = NumberSequence.new{NumberSequenceKeypoint.new(0,0,0),NumberSequenceKeypoint.new(1,1,0)};
		Width0 = 1.1;
...
local lplrCharacterAdded = LPH_JIT_MAX(function(character)
	...
	local particle_connection = nil
	local ammo_connection = nil
	lplr_data["gun"] = nil
	local sound_connection = nil

	utility.newConnection(character.ChildAdded, LPH_JIT_MAX(function(object)
		lplr_parts[object.Name] = object
		if object.ClassName == "Tool" then
			...
			if handle then
				...
				sound_connection = utility.newConnection(handle.ChildAdded, LPH_JIT_MAX(function(sound)
					if sound.ClassName == "Sound" and sound.Name == "MG" then
						if flags["custom_shoot_sounds"] then
							local shoot_sound = flags["custom_shoot_id"][1]
							sound.Volume = flags["shoot_volume"]
							sound.SoundId = shoot_sound == "Custom" and (isfile(flags["shoot_id"]) and getcustomasset(flags["shoot_id"], false) or flags["shoot_id"]) or shootsounds[flags["shoot_sound"][1]];
						elseif flags["skin_changer"] then
							local skin = skins["equipped"][object.Name]
							local skin_object = skins[skin] 
							if skin and skin_object then
								local shoot_sounds = skin_object.ShootSounds
								if shoot_sounds then
									local id = shoot_sounds[utility.removeBrackets(object.Name)]
									if id then
										sound.SoundId = id
									end
								end
				...
				local sound = handle:FindFirstChild("ShootSound")
			...
			if flags["skin_changer"] then
				local skin = skins["equipped"][object.Name]
				if skin then
					task.delay(0, function()
						applySkin(object, skin, object.Name == "[Knife]")
					end)
				end
			end
			...
			if ammo then
				...
				local shootbbgui = object.Handle:FindFirstChild("ShootBBGUI")

				if shootbbgui then
					particle_connection = utility.newConnection(shootbbgui.Shoot:GetPropertyChangedSignal("Visible"), LPH_JIT_MAX(function()
						local skin = skins["equipped"][object.Name]
						if skin and skins[skin].ShotParticle then
							shootbbgui.Shoot.Visible = false
						end
					end), true)
				end
				...
				activate_connection = utility.newConnection(object.Activated, LPH_JIT_MAX(function()
					...
					if aim_location and flags["anti_aim_viewer"] then
						event:FireServer("UpdateMousePos", flags["aim_backtrack"] and getClosestPosition(aim_location) or aim_location)
					end
				end), true)
	...
	utility.newConnection(character.ChildRemoved, LPH_JIT_MAX(function(object)
		...
		if object.ClassName == "Tool" then
			...
			if particle_connection then
				particle_connection:Disconnect()
			end
			if sound_connection then
				sound_connection:Disconnect()
			end
			lplr_data["gun"] = nil
			lplr_data["tool"] = nil
	...
	utility.newConnection(humanoid:GetPropertyChangedSignal("MoveDirection"), LPH_JIT_MAX(function(direction)
	...
	utility.newConnection(knocked:GetPropertyChangedSignal("Value"), function()
		if aimbot.target and find(flags["untarget"], "Self knocked") then
			aimbot.target = nil
			aimbotTargetChange:Fire(nil)
		end
	end, true)
	...
	utility.newConnection(armor:GetPropertyChangedSignal("Value"), LPH_JIT_MAX(function()
		lplr_data["armor"] = armor.Value
	...
	if flags["game_hud_changer"] then
		hud_styles[flags["game_hud_style"][1]]()
		local bar = main_gui.Bar
		bar.HP.bar.BackgroundColor3 = flags["hud_health_color"]
		bar.Energy.bar.BackgroundColor3 = flags["hud_energy_color"]
		bar.Armor.bar.BackgroundColor3 = flags["hud_armor_color"]
		bar.Armor.firebar.BackgroundColor3 = flags["hud_firearmor_color"]
	end

	if flags["money_text_changer"] then
		local text = main_gui:WaitForChild("MoneyText")
	
		text.Font = Enum.Font[flags["money_text_font"][1]]
		text.TextSize = flags["money_text_size"]
		text.TextStrokeTransparency = flags["money_text_outline_transparency"]
		text.TextStrokeColor3 = flags["money_text_outline_color"]
		text.TextColor3 = flags["money_text_color"]
		text.TextTransparency = flags["money_text_transparency"]
	
		local button = main_gui:WaitForChild("Promote")

		button.Visible = not flags["hide_buy_button"]
	end
...
_spawn(function()
	local data_folder = lplr:WaitForChild("DataFolder")
		local information = data_folder:WaitForChild("Information")
			local wanted = information:WaitForChild("Wanted", 30)

			if wanted then
				local old_wanted = wanted.Value

				utility.newConnection(wanted:GetPropertyChangedSignal("Value"), function()
					if wanted.Value-old_wanted == 50 then
						lplr_data["recently_stomped"] = true
					end
				end, true)
			end
...
local anti_aim = window:getTab(2)
	...
	local other_anti = anti_aim:newSection({name = "Other", is_changeable = true, x = 0.5, scale = 1})
		menu_references["anti_rpg"] = other_anti:newElement({name = "Anti rpg", types = {toggle = {flag = "anti_rpg"}}})
local legit = window:getTab(3)
	local aimbot_section = legit:newSection({name = "Aimbot settings", is_changeable = false, scale = 0.7})
		menu_references["aimbot_toggle"] = aimbot_section:newElement({name = "Enabled", types = {toggle = {flag = "aimbot"}, keybind = {flag = "aimbot_keybind", method = 1}}}); keybinder:add(menu_references["aimbot_toggle"], "Aimbot", "aimbot_keybind", "aimbot_toggle")
		...
		menu_references["rocket_tp"] = aimbot_section:newElement({name = "Rocket tp", types = {toggle = {flag = "rocket_tp"}}})
		menu_references["use_prediction"] = aimbot_section:newElement({name = "Use silent aim location", types = {toggle = {flag = "use_prediction"}}})
		...
		aimbot_section:newElement({name = "Target checks", types = {dropdown = {flag = "target_checks", multi = true, options = {"Behind wall", "Forcefield", "Grabbed", "Knocked", "Crew"}}}})
...
local visuals = window:getTab(4)
	local player_esp_section = visuals:newSection({name = "Player ESP", is_changeable = true, scale = 0.6})
		...
		menu_references["esp_font"] = player_esp_section:newElement({name = "Text font", types = {dropdown = {flag = "esp_font", no_none = true, default = {"1"}, options = {"1", "2", "3", "4"}}}})
	...
	local hud_section = visuals:newSection({name = "Hud", is_changeable = true, x = 0.5, y = 0.3, scale = 0.35})
		...
		menu_references["drawing_crosshair_length"] = hud_section:newElement({name = "Length", types = {slider = {flag = "drawing_crosshair_length", suffix = "px", prefix = "", min = 1, max = 100}}})
		menu_references["drawing_crosshair_gap"] = hud_section:newElement({name = "Gap", types = {slider = {flag = "drawing_crosshair_gap", suffix = "px", prefix = "", min = 1, max = 100}}})
		...
		menu_references["target_font"] = hud_section:newElement({name = "Font", types = {dropdown = {options = {"1", "2", "3", "4"}, no_none = true, flag = "target_font", default = {"1"}}}})
		menu_references["target_font"]:setVisible(false)
		menu_references["target_style"]:setVisible(false)
		menu_references["money_text_changer"] = hud_section:newElement({name = "Money text changer", types = {toggle = {flag = "money_text_changer"}, colorpicker = {flag = "money_text_color", transparency_flag = "money_text_transparency", default = colorfromrgb(95, 255, 87)}}})
		menu_references["hide_buy_button"] = hud_section:newElement({name = "Hide buy button", types = {toggle = {flag = "hide_buy_button"}}})
		menu_references["money_text_outline"] = hud_section:newElement({name = "Text outline", types = {colorpicker = {flag = "money_text_outline_color", default = colorfromrgb(0,0,0), default_transparency = 1, transparency_flag = "money_text_outline_transparency"}}})
		menu_references["money_text_font"] = hud_section:newElement({name = "Text font", types = {dropdown = {flag = "money_text_font", options = {"Gotham", "RobotoMono", "SourceSans"}, no_none = true, default = {"Gotham"}}}})
		menu_references["money_text_size"] = hud_section:newElement({name = "Text size", types = {slider = {min = 14, default = 25, max = 50, suffix = "px", prefix = "", flag = "money_text_size"}}})
		menu_references["game_hud_changer"] = hud_section:newElement({name = "Game hud changer", types = {toggle = {flag = "game_hud_changer"}, dropdown = {flag = "game_hud_style", no_none = true, default = {"Default"}, options = {"Default", "Minimalistic", "Simple dark", "Gamesense"}}}})
		menu_references["hud_health_color"] = hud_section:newElement({name = "Health bar color", types = {colorpicker = {flag = "hud_health_color", transparency_flag = "hud_health_transparency", default = colorfromrgb(118, 237, 107), default_transparency = 0.5}}})
		menu_references["hud_energy_color"] = hud_section:newElement({name = "Energy bar color", types = {colorpicker = {flag = "hud_energy_color", transparency_flag = "hud_energy_transparency", default = colorfromrgb(245, 213, 54), default_transparency = 0.5}}})
		menu_references["hud_armor_color"] = hud_section:newElement({name = "Armor bar color", types = {colorpicker = {flag = "hud_armor_color", transparency_flag = "hud_armor_transparency", default = colorfromrgb(107, 215, 227), default_transparency = 0.5}}})
		menu_references["hud_firearmor_color"] = hud_section:newElement({name = "Fire armor bar color", types = {colorpicker = {flag = "hud_firearmor_color", transparency_flag = "hud_firearmor_transparency", default = colorfromrgb(255, 167, 79), default_transparency = 0.5}}})
		...
		menu_references["notification_font"] = hud_section:newElement({name = "Font", types = {dropdown = {options = {"1", "2", "3", "4"}, no_none = true, flag = "notification_font", default = {"1"}}}})
		...
		menu_references["keybinds_font"] = hud_section:newElement({name = "Font", types = {dropdown = {options = {"1", "2", "3", "4"}, no_none = true, flag = "keybinds_font", default = {"1"}}}})
		menu_references["watermark"] = hud_section:newElement({name = "Watermark", types = {toggle = {flag = "watermark"}, colorpicker = {flag = "watermark_color", default = colorfromrgb(153, 196, 39), transparency_flag = "watermark_transparency"}}})
		menu_references["watermark_font"] = hud_section:newElement({name = "Font", types = {dropdown = {options = {"1", "2", "3", "4"}, no_none = true, flag = "watermark_font", default = {"1"}}}})
		...
		menu_references["watermark_smoothness"] = hud_section:newElement({name = "Smoothness", types = {slider = {flag = "watermark_smoothness", suffix = "%", prefix = "", min = 0, max = 100}}})
	...
	local other_section = visuals:newSection({name = "Other", is_changeable = true, x = 0.5, y = 0.65, scale = 0.35})
		menu_references["custom_shoot_sounds"] = other_section:newElement({name = "Custom shoot sounds", types = {toggle = {flag = "custom_shoot_sounds"}, dropdown = {flag = "custom_shoot_id", no_none = true, default = {"SSG08"}, options = {"SSG08", "SCAR20", "AWP", "Custom"}}}})
		menu_references["shoot_id"] = other_section:newElement({name = "Custom ID", types = {textbox = {flag = "shoot_id"}}})
		menu_references["shoot_volume"] = other_section:newElement({name = "Volume", types = {slider = {flag = "shoot_volume", min = 0.1, max = 5, suffix = "", decimal = 1, default = 1, prefix = ""}}})
		...
		menu_references["local_bullet_tracers"] = other_section:newElement({name = "Local bullet tracers", types = {toggle = {flag = "local_bullet_tracers"}, colorpicker = {flag = "local_bullet_tracers_color", transparency_flag = "local_bullet_tracers_transparency"}, dropdown = {flag = "local_bullet_tracers_texture", options = {"Electric", "Laser", "Flash", "Flow"}, default = {"Laser"}, no_none = true}}})
		...
		menu_references["enemy_bullet_tracers"] = other_section:newElement({name = "Enemy bullet tracers", types = {toggle = {flag = "enemy_bullet_tracers"}, colorpicker = {flag = "enemy_bullet_tracers_color", transparency_flag = "enemy_bullet_tracers_transparency"}, dropdown = {flag = "enemy_bullet_tracers_texture", options = {"Electric", "Laser", "Flash", "Flow"}, default = {"Laser"}, no_none = true}}})
		...
		menu_references["damage_number"] = other_section:newElement({name = "Damage number", types = {toggle = {flag = "damage_number"}, colorpicker = {flag = "damage_number_color", transparency_flag = "damage_number_transparency"}}})
		menu_references["damage_number_font"] = other_section:newElement({name = "Font", types = {dropdown = {flag = "damage_number_font", options = {"1", "2", "3", "4"}, default = {"1"}, no_none = true}}})
		menu_references["damage_number_size"] = other_section:newElement({name = "Size", types = {slider = {flag = "damage_number_size", min = 10, max = 30, suffix = "", decimal = 0, default = 24, prefix = ""}}})
local misc = window:getTab(5)
	...
	local game_section = misc:newSection({name = "Game modifiers", is_changeable = true, y = 0.5, scale = 0.5})
		...
		utility.newConnection(game_section:newElement({name = "No flashbang", types = {toggle = {flag = "no_flashbang"}}}).onToggleChange, function(bool)
			...
			if bool then
				flashbang_connection = utility.newConnection(main_gui.ChildAdded, LPH_JIT_MAX(function(object)
	...
	local utilities_section = misc:newSection({name = "Utilities", is_changeable = true, x = 0.5, scale = 0.5})
		...
		menu_references["cash_aura"] = utilities_section:newElement({name = "Cash aura", types = {toggle = {flag = "cash_aura"}}})
...
local skins_tab = window:getTab(6)
	local purchases_section = skins_tab:newSection({name = "Purchases", is_changeable = true, scale = 1})
		...
		menu_references["extra_delay"] = purchases_section:newElement({name = "Extra delay", types = {slider = {min = 0, max = 50, suffix = "ms", prefix = "", flag = "purchase_delay"}}})
		purchases_section:newElement({name = "Purchase", types = {button = {text = "Purchase", callback = function()
			...
			_spawn(purchaseItem, menu_references["purchases"].selected_option)
		end}}})
	local skins_section = skins_tab:newSection({name = "Skins", is_changeable = true, x = 0.5, scale = 1})
		...
		utility.newConnection(menu_references["selected_weapon"].onDropdownChange, function(selected)
			...
			for _, skin in skin_table[selected[1]] do
				menu_references["weapon_skin"]:addOption(_)
			end
		...
		skins_section:newElement({name = "Refresh custom skins", types = {button = {text = "Refresh custom skins", callback = function()
			for _, file in listfiles(config_location.."/skins/") do
				local data = readfile(file)
				local json = nil
				local s, err = pcall(function()
					json = hs:JSONDecode(data)
				end)
				...
				if skin_table["[Revolver]"][name] and not custom_skins[name]  then
					continue end

				skins[name] = {
					Color = ColorSequence.new{
						ColorSequenceKeypoint.new(0.00, json["StartBeamColor"] and colorfromrgb(unpack(json["StartBeamColor"])) or colorfromrgb(255,255,255)),
						ColorSequenceKeypoint.new(1.00, json["EndBeamColor"] and colorfromrgb(unpack(json["EndBeamColor"])) or colorfromrgb(255,255,255)),
					},
				}

				if json["Particle"] then
					local object = nil
					local s, err = pcall(function()
						object = game:GetObjects(json["Particle"])
					end)
				...
				skin_table[json["Gun"]][name] = {
					TextureID = json["TextureID"] or ""
				}
...
local players_tab = window:getTab(7)
	local players_section = players_tab:newSection({name = "Players", is_changeable = true, x = 0, scale = 1})
		...
		menu_references["auto_stomping"] = players_section:newElement({name = "Auto stomp", types = {toggle = {no_load = true, flag = "auto_stomping"}}})
...
local configuration = window:getTab(8)
		...
		config_section:newElement({name = "Load config", types = {button = {text = "Load config",
			callback = function()
				if config_list.selected_option then
					...
					if flags["equipped_skins"] then
						skins["equipped"] = flags["equipped_skins"]
					end
	...
	local lua_section = configuration:newSection({name = "LUA", is_changeable = false, x = 0.5, scale = 1})
		...
		getgenv().juju = {
			...
			flags = flags,
			section = lua_section,
			purchase_item = LPH_NO_VIRTUALIZE(function(item)
				local shop = shop_items[item]

				if not shop then
					return error("juju.lol\n	tried to purchase invalid item\""..item.."\"") end
		...
		lua_section:newElement({name = "Load script", types = {button = {text = "Load script",
			callback = function()
				if script_list.selected_option and not script_environment[script_list.selected_option] then
					...
					if not find(flags["loaded_scripts"], name) then
						insert(flags["loaded_scripts"], name)
					end
...
utility.newConnection(menu_references["hide_buy_button"].onToggleChange, LPH_JIT_MAX(function(bool)
	if not flags["money_text_changer"] then
		return end

	if not main_gui then
		return end

	local button = main_gui:FindFirstChild("Promote")

	if not button then
		return end

	button.Visible = not bool
end), true)

utility.newConnection(menu_references["money_text_outline"].onColorChange, LPH_JIT_MAX(function(color, transparency)
	if not flags["money_text_changer"] then
		return end
	...
	local text = main_gui:FindFirstChild("MoneyText")
...
utility.newConnection(menu_references["money_text_changer"].onColorChange, LPH_JIT_MAX(function(color, transparency)
	if not flags["money_text_changer"] then
		return end
...
utility.newConnection(menu_references["money_text_size"].onSliderChange, LPH_JIT_MAX(function(amount)
	if not flags["money_text_changer"] then
		return end
	...
	text.TextSize = amount
end), true)

utility.newConnection(menu_references["money_text_font"].onDropdownChange, LPH_JIT_MAX(function(amount)
	if not flags["money_text_changer"] then
		return end
...
utility.newConnection(menu_references["money_text_changer"].onToggleChange, LPH_JIT_MAX(function(bool)
	menu_references["hide_buy_button"]:setVisible(bool)
	menu_references["money_text_outline"]:setVisible(bool)
	menu_references["money_text_font"]:setVisible(bool)
	menu_references["money_text_size"]:setVisible(bool)
	...
	text.Font = bool and Enum.Font[flags["money_text_font"][1]] or Enum.Font.GothamMedium
	text.TextSize = bool and flags["money_text_size"] or 25
	text.TextStrokeTransparency = bool and flags["money_text_outline_transparency"] or 1
	text.TextStrokeColor3 = bool and flags["money_text_outline_color"] or colorfromrgb(0,0,0)
	text.TextColor3 = bool and flags["money_text_color"] or colorfromrgb(95, 255, 87)
	text.TextTransparency = bool and flags["money_text_transparency"] or 0
	...
	button.Visible = bool and (not flags["hide_buy_button"]) or true
end), true)
menu_references["hide_buy_button"]:setVisible(false)
menu_references["money_text_outline"]:setVisible(false)
menu_references["money_text_font"]:setVisible(false)
menu_references["money_text_size"]:setVisible(false)
...
do
	...
	utility.newConnection(gradient_glow:GetPropertyChangedSignal("Size"), LPH_NO_VIRTUALIZE(function()
		...
		if gradient_glow.Size.Y < 0.01 then
			...
			gradient_glow.Parent = workspace.Ignored
		end
	end), true)
end
...
utility.newConnection(menu_references["gradient_overlay"].onToggleChange, function(bool)
	...
	gradient_glow.Parent = (bool and aimbot.target) and workspace.Ignored or cg
end, true)
...
utility.newConnection(camera:GetPropertyChangedSignal("ViewportSize"), function()
	local position = camera.ViewportSize/2
	aimbot.circle.Position = position
	aimbot.circle_outline.Position = position
	aimbot.assist_circle_outline.Position = position
	aimbot.assist_circle.Position = position
end, true)

utility.newConnection(menu_references["damage_number"].onToggleChange, function(bool)
	menu_references["damage_number_font"]:setVisible(bool)
	menu_references["damage_number_size"]:setVisible(bool)
end)
menu_references["damage_number_font"]:setVisible(false)
menu_references["damage_number_size"]:setVisible(false)
...
utility.newConnection(menu_references["silent_aim"].onToggleChange, function(bool)
	...
	menu_references["rocket_tp"]:setVisible(bool)
	menu_references["use_prediction"]:setVisible(bool and flags["rocket_tp"] or false)
end)
...
menu_references["rocket_tp"]:setVisible(false)
menu_references["use_prediction"]:setVisible(false)
...
utility.newConnection(menu_references["gradient_bars"].onToggleChange, function(bool)
	...
	for player, info in player_data do
		...
		if drawings then
			...
			local ammo_bar = drawings[8] 

			if ammo_bar then
				ammo_bar["Data"] = image
			end

			local armor_bar = drawings[11] 

			if armor_bar then
				armor_bar["Data"] = image
			end
		end
	end
end)
...
utility.newConnection(aimbotTargetChange, LPH_NO_VIRTUALIZE(function(target)
	...
	gradient_glow.Parent = (flags["gradient_overlay"] and target) and workspace.Ignored or cg
	...
	if not checkcaller() then
		if spoof_properties[index] then
			...
			if flags[skip_indexes[index]] then
				...
				return
			elseif index == "CFrame" and self == camera and flags["no_recoil"] and getcallingscript()["Name"] == "Framework" then
				return
			end
		end
	end
...
do
	local rocketTp = LPH_NO_VIRTUALIZE(function()
		...
		local target_position = flags["use_prediction"] and aimbot.target_position or (aimbot.target and player_data[aimbot.target]["character_parts"]["HumanoidRootPart"].Position)

		if tool.Name == "[RPG]" and target_position then
			for _, object in workspace.Ignored:GetChildren() do
				if object.Name == "Model" then
					local part = object:FindFirstChild("Launcher")
					if part then
						local body_velocity = part:FindFirstChild("BodyVelocity")
						if body_velocity then
							body_velocity:Destroy()
						end
						local touch_interest = part:FindFirstChild("TouchInterest")
						if touch_interest then
							touch_interest:Destroy()
						end
						part.CFrame = cframenew(target_position + vector3new(0,mathrandom(-1,1),0))
						part.Velocity = vector3new(0,0.01,0)
					end
				end
			end
		end
	...
	utility.newConnection(menu_references["rocket_tp"].onToggleChange, LPH_NO_VIRTUALIZE(function(bool)
		if find(heartbeat_callbacks, rocketTp) then
			remove(heartbeat_callbacks, rocketTp)
		end
		if bool then
			insert(heartbeat_callbacks, rocketTp)
		end
		menu_references["use_prediction"]:setVisible(bool)
	end), true)
end
...
do
	...
	local doCrosshair = LPH_JIT_MAX(function(dt)
		...
		local smooth = flags["crosshair_smoothness"] == 0 and 1 or dt*(20*(1.25-flags["crosshair_smoothness"]/100))
		for i = 1, 4 do 
			...
			line.From = lerp(line.From, location + pos * gap, smooth)
			line.To = lerp(line.To, line.From + pos * length, smooth)
			outline.From = lerp(outline.From, location + pos * (gap - 1), smooth)
			outline.To = lerp(outline.To, outline.From + pos * (length + 1), smooth)
			line.Visible = true 
	...
	utility.newConnection(menu_references["drawing_crosshair"].onToggleChange, LPH_NO_VIRTUALIZE(function(bool)
		...
		menu_references["crosshair_smoothness"]:setVisible(bool)
	...
	menu_references["drawing_crosshair_spin"]:setVisible(false)
	menu_references["crosshair_smoothness"]:setVisible(false)
...
do
	...
	utility.newConnection(menu_references["trail"].onToggleChange, function(bool)
		part.Parent = bool and workspace.Ignored or cg
...
do
	...
	doWatermark = LPH_NO_VIRTUALIZE(function(dt)
		...
		local smooth = flags["watermark_smoothness"] == 0 and 1 or dt*(20*(1.25-flags["watermark_smoothness"]/100))
		pos+=vector2new(flags["watermark_x_offset"], flags["watermark_y_offset"]) - vector2new(total_size, 0)

		watermark[1]["Position"] = lerp(watermark[1]["Position"], pos, smooth)
		watermark[2]["Position"] = lerp(watermark[2]["Position"], pos + vector2new(watermark[1]["TextBounds"]["X"],0), smooth)
	...
	utility.newConnection(menu_references["watermark"].onToggleChange, function(bool)
		...
		menu_references["watermark_smoothness"]:setVisible(bool)
	end, true)
	...
	menu_references["watermark_smoothness"]:setVisible(false)
end
...
do
	...
	local do_priority_flag = flags["priority_flag"]
	local do_friendly_flag = flags["friendly_flag"]
...
local doCashAura = LPH_NO_VIRTUALIZE(function()
	...
	for _, cash in workspace.Ignored.Drop:GetChildren() do
...
local doAimbot = LPH_NO_VIRTUALIZE(function(dt)
	...
	if (target or flags["auto_select_target"]) and flags["aimbot_keybind"]["active"] then
		...
		if not stop and target then
			...
			if not local_hrp or not hrp then
				aimbot.target_position = nil
				line.Visible = false
				line_outline.Visible = false
				if body and body.Parent == workspace.Ignored then
			...
			if flags["resolver"] then
				old_velocity = aimbot.target_velocity or old_velocity
			end
			...
			if flags["silent_aim"] then
				...
				for _, check in flags["silent_aim_disable_when"] do
					...
					elseif check == "Behind wall" then
						local raycast_params = RaycastParams.new()
						raycast_params.IgnoreWater = true
						raycast_params.FilterDescendantsInstances = {target.Character, char, workspace.Ignored}
						raycast_params.FilterType = Enum.RaycastFilterType.Exclude
				...
				if not stop then
					...
					if flags["position_line"] then
						...
						if flags["line_position"][1] == "Predicted position" then
							local _pos, on_screen = wtvp(camera, aimbot_position)
							if on_screen then
								destination = vector2new(_pos.X, _pos.Y)
							end
						else
							local _pos, on_screen = wtvp(camera, part_position)
							if on_screen then
								destination = vector2new(_pos.X, _pos.Y)
							end
						end
			...
			if flags["aim_assist"] and flags["aim_assist_keybind"]["active"] then
				...
				for _, check in flags["aim_assist_disable_when"] do
					if check == "Not holding gun" and not lplr_data["gun"] then
						stop = true
						break
				...
				if not stop then
					...
					if flags["aim_assist_style"][1] == "Mouse" then
						...
						if aimbot.do_tp then
							...
							local horizontal = flags[gun.."_aim_assist_horizontal_shake"]/2.5
							local vertical = flags[gun.."_aim_assist_vertical_shake"]/2.5
							local add = vector2new()
							if horizontal > 0 then
								add+=vector2new(mathrandom(-horizontal,horizontal),0)
							end
							if vertical > 0 then
								add+=vector2new(0,mathrandom(-vertical,vertical))
							end
							if add ~= vector2new() then
								mousemoverel(add.X, add.Y)
							end
						end
					else
						if aimbot.do_tp then
							...
							local horizontal = flags[gun.."_aim_assist_horizontal_shake"]/25
							local vertical = flags[gun.."_aim_assist_vertical_shake"]/25
							local velocity = vector3new()
							if horizontal > 0 then
								velocity+=vector3new(mathrandom(-horizontal,horizontal),0,mathrandom(-horizontal,horizontal))
							end
							if vertical > 0 then
								velocity+=vector3new(0,mathrandom(-vertical,vertical),0)
							end
							camera.CFrame*=angles(rad(velocity.Y),rad(velocity.X),0)
						end
					end
...
local renderRPGWarnings = LPH_NO_VIRTUALIZE(function()
	...
	for _, warning in rpg_indicators do
		...
		if new_tick-last_refresh > 0.175 then
			warning[7] = new_tick

			local params = RaycastParams.new()
			params.FilterType = Enum.RaycastFilterType.Exclude
			params.FilterDescendantsInstances = {workspace.Ignored, rocket.Parent}

			local lookVector = rocket.CFrame.lookVector
		
			local result = raycast(workspace, rocket.Position, lookVector*-1000, params)
	
			if result then
				local raycast_params = RaycastParams.new()
				raycast_params.IgnoreWater = true;
				raycast_params.FilterDescendantsInstances = {char, rocket.Parent, camera, workspace.Vehicles, workspace.Ignored, workspace.Players}
				raycast_params.FilterType = Enum.RaycastFilterType.Exclude

				warning[5] = result.Position
				warning[6] = raycast(workspace, result.Position + lookVector*1, (hrp.Position - result.Position).unit*(hrp.Position - result.Position).magnitude, raycast_params) == nil
				visible = warning[5]
				pos = result.Position
			end
		end
		...
		if hrp then
			...
			local screen_pos, on_screen = wtvp(camera, pos)
			screen_pos = vector2new(screen_pos.X, screen_pos.Y)
			if on_screen then
				if visible then
					local size = floor(((screen_pos.Y - wtvp(camera, pos + vector3new(0, 2.8, 0)).Y)/2)*2.5)
					image["Visible"] = true
					...
					image["Color"] = Color3.new(0,1,0):Lerp(Color3.new(1,0,0), 30/(hrp.Position-pos).magnitude)
					distance["Visible"] = true
					distance["Size"] = size/3
					distance["Text"] = tostring(round((rocket.Position-pos).magnitude/50, 1)).."s"
...
local autoShoot = LPH_NO_VIRTUALIZE(function()
	...
	local raycast_params = RaycastParams.new()
	raycast_params.IgnoreWater = true;
	raycast_params.FilterDescendantsInstances = {char, target.Character, camera, workspace.Ignored, workspace.Vehicles}
	raycast_params.FilterType = Enum.RaycastFilterType.Exclude
...
local rpg_added_connection2 = nil
...
utility.newConnection(menu_references["anti_rpg"].onToggleChange, function(bool)
	if rpg_added_connection2 then
		rpg_added_connection2:Disconnect()
	end
	if bool then
		rpg_added_connection2 = utility.newConnection(workspace.Ignored.ChildAdded, LPH_NO_VIRTUALIZE(function(object)
			if object.Name == "Model" then
				local part = object:WaitForChild("Launcher", 1)
				if part then
					_spawn(avoidRPG, part)
				end
			end
		end), true)
	end
...
utility.newConnection(menu_references["rpg_warnings"].onToggleChange, function(bool)
	...
	if find(heartbeat_callbacks, renderRPGWarnings) then
		remove(heartbeat_callbacks, renderRPGWarnings)
	end
	if bool then
		insert(heartbeat_callbacks, renderRPGWarnings)
		rpg_added_connection = utility.newConnection(workspace.Ignored.ChildAdded, LPH_NO_VIRTUALIZE(function(object)
			if object.Name == "Model" then
				local part = object:WaitForChild("Launcher", 1)
				if part then
					local params = RaycastParams.new()
					params.FilterType = Enum.RaycastFilterType.Exclude
					params.FilterDescendantsInstances = {workspace.Ignored, workspace.Vehicles}

					local lookVector = part.CFrame.lookVector
				
					local result = raycast(workspace, part.Position, lookVector*-1000, params)
			
					if result then
						_spawn(utility.newRPGIndicator, part, result.Position + lookVector*-1)
					end
				end
			end
		end), true)
...
utility.newConnection(menu_references["resolver"].onToggleChange, function(bool)
	...
	menu_references["not_on_vehicles"]:setVisible(bool)
end, true)
menu_references["resolver_refresh"]:setVisible(false)
menu_references["not_on_vehicles"]:setVisible(false)
...
utility.newConnection(workspace.Ignored.Siren.Radius.ChildAdded, LPH_NO_VIRTUALIZE(function(object)
	_wait()
...
utility.newConnection(menu_references["custom_shoot_sounds"].onDropdownChange, function(selected)
	menu_references["shoot_id"]:setVisible(flags["custom_shoot_sounds"] and selected[1] == "Custom" or false)
end, true)

utility.newConnection(menu_references["custom_shoot_sounds"].onToggleChange, function(bool)
	menu_references["shoot_volume"]:setVisible(bool)
	menu_references["shoot_id"]:setVisible(bool and flags["custom_shoot_id"][1] == "Custom" or false)
end, true)
menu_references["shoot_volume"]:setVisible(false)
menu_references["shoot_id"]:setVisible(false)
...
local renderBulletImpact = LPH_JIT(function(object, beam, position)
	...
	_wait(flags["local_bullet_impacts_lifetime"])

	tween(outline, newtweeninfo(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Transparency = 1})
	tween(impact, newtweeninfo(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Transparency = 1})
...
local renderBulletTracer = LPH_JIT(function(object, beam, is_lplr)
	local end_position = beam.Attachment1.WorldCFrame
	local start_position = object.CFrame

	local beam = beam_styles[is_lplr and flags["local_bullet_tracers_texture"][1] or flags["enemy_bullet_tracers_texture"][1]]:Clone()
	...
	_wait(is_lplr and flags["local_bullet_tracers_lifetime"] or flags["enemy_bullet_tracers_lifetime"])

	tween = utility.newConnection(rs.Heartbeat, function(dt)
		total_time+=dt
		beam.Transparency = NumberSequence.new(getValue(tws, (total_time / 0.2), Enum.EasingStyle.Quad, Enum.EasingDirection.Out));
	end, true)
...
local spinningCrosshair = LPH_JIT(function(dt)
...
local autoStomp = LPH_NO_VIRTUALIZE(function(dt)
	local hrp = lplr_parts["HumanoidRootPart"]
...
utility.newConnection(onBulletFired, LPH_JIT_MAX(function(object, beam, is_lplr, hit)
	if is_lplr then
		if flags["skin_changer"] then
			local tool = lplr_data["tool"]
			if tool then
				local skin = skins["equipped"][tool.Name]
				local skin_object = skins[skin] 
				if skin and skin_object then
					local color = skin_object.Color
					if color then
						beam.Color = color
					end
		...
		if flags["local_bullet_impacts"] then
			_spawn(renderBulletImpact, object, beam, hit)
		end
		if flags["local_bullet_tracers"] then
			_spawn(renderBulletTracer, object, beam, is_lplr)
		end
	else
...
local renderDamageNumber = LPH_NO_VIRTUALIZE(function(player, damage)
	local data = player_data[player]

	local hrp = data.character_parts["Head"]
	...
	local text = utility.newDrawing("Text", {
		Size = flags["damage_number_size"],
		Font = fonts[tonumber(flags["damage_number_font"][1])],
	...
	local connection = nil; connection = utility.newConnection(rs.Heartbeat, function(dt)
		...
		if on_screen then
			text["Position"] = vector2new(pos.X, pos.Y)
			text["Visible"] = true
		else
			text["Visible"] = false
		end
		...
		if elapsed_time > 0.15 then
			if elapsed_time < 0.65 then
				new_value = old_value + (value-old_value)*getValue(tws, (elapsed_time-0.15 / 0.5), Enum.EasingStyle.Circular, Enum.EasingDirection.Out)
				text.Transparency = new_value
				position+=vector3new(0,3.1*dt,0)
			else
				connection:Disconnect()
				text:Destroy()
			end
...
utility.newConnection(lplrHitPlayer, LPH_JIT_MAX(function(player, damage)
	...
	if flags["notifications"] and find(flags["notifications_selected"], "Hits") then
		_spawn(newNotification, "hit "..player.Name.." for "..damage.. " damage", flags["notification_style"][1])
	end
	...
	if flags["damage_number"] then
		_spawn(renderDamageNumber, player, damage)
	end
...
utility.newConnection(menu_references["spinning_crosshair"].onToggleChange, function(bool)
	...
	if find(heartbeat_callbacks, spinningCrosshair) then
...
utility.newConnection(menu_references["hit_effect"].onColorChange, function(color)
	for _, particle in particle_part:GetChildren() do
		particle.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, color), ColorSequenceKeypoint.new(1.00, color)}
	end
end, true)
menu_references["hit_effect"]:setColor(colorfromrgb(255,0,0), 0, true)
...
utility.newConnection(menu_references["auto_stomp"].onToggleChange, function(bool)
	if find(heartbeat_callbacks, autoStomp) then
...
local cframeSpeed = LPH_JIT_MAX(function(dt)
...
local cframeFly = LPH_JIT_MAX(function(dt)
...
local targetStrafe = LPH_JIT_MAX(function(dt)
...
utility.newConnection(menu_references["aimbot_priority"].onToggleChange, LPH_JIT_MAX(function(bool)
	local player = plrs:FindFirstChild(menu_references["players_box"]["selected_option"])
...
local autoStomping = LPH_NO_VIRTUALIZE(function()
	local auto_stomping = lplr_data["auto_stomping"]

	if not auto_stomping or (not lplr_data["auto_stomping_true"] and lplr_data["force_cframe"]) then
		lplr_data["auto_stomping_true"] = false
		return 
	end

	local data = player_data[auto_stomping]
	...
	if not data["knocked"] then
		lplr_data["auto_stomping_true"] = false
		lplr_data["force_cframe"] = nil
		return 
	end

	local upper_torso = data.character_parts["UpperTorso"]
	...
	local body_effects = data.character_parts["BodyEffects"]
	...
	lplr_data["auto_stomping_true"] = true
	lplr_data["force_cframe"] = cframenew(upper_torso.Position + vector3new(0,2.5,0))
...
utility.newConnection(menu_references["auto_stomping"].onToggleChange, LPH_JIT_MAX(function(bool)
	local player = plrs:FindFirstChild(menu_references["players_box"]["selected_option"])

	lplr_data["auto_stomping"] = player or nil

	if player and not find(heartbeat_callbacks, autoStomping) then
		insert(heartbeat_callbacks, autoStomping)
	elseif find(heartbeat_callbacks, auto_stomping) then
		lplr_data["force_cframe"] = nil
		lplr_data["auto_stomping_true"] = false
		remove(heartbeat_callbacks, autoStomping)
	end
end), true)

utility.newConnection(menu_references["whitelisted"].onToggleChange, LPH_JIT_MAX(function(bool)
	local player = plrs:FindFirstChild(menu_references["players_box"]["selected_option"])
...
utility.newConnection(menu_references["players_box"].onSelectionChange, LPH_JIT_MAX(function(selected)
	...
	menu_references["whitelisted"]:setToggle(lplr_data["auto_stomping"] == player, true)
end), true)
...
menu_references["armor_threshold"]:setVisible(false)
...
menu_references["fire_threshold"]:setVisible(false)
...
utility.newConnection(menu_references["hud_health_color"].onColorChange, function(color, transparency)
	if not main_gui or not flags["game_hud_changer"] then
		return end
	...
	bar.HP.bar.BackgroundColor3 = color
end)

utility.newConnection(menu_references["hud_firearmor_color"].onColorChange, function(color, transparency)
	if not main_gui or not flags["game_hud_changer"] then
		return end
	...
	bar.Armor.firebar.BackgroundColor3 = color
end)

utility.newConnection(menu_references["hud_energy_color"].onColorChange, function(color, transparency)
	if not main_gui or not flags["game_hud_changer"] then
		return end
	...
	bar.Energy.bar.BackgroundColor3 = color
end)

utility.newConnection(menu_references["hud_armor_color"].onColorChange, function(color, transparency)
	if not main_gui or not flags["game_hud_changer"] then
		return end
	...
	bar.Armor.bar.BackgroundColor3 = color
end)

utility.newConnection(menu_references["game_hud_changer"].onDropdownChange, function(selected)
	if flags["game_hud_changer"] then
		hud_styles["Default"]()
		hud_styles[selected[1]]()
	end
end)

utility.newConnection(menu_references["game_hud_changer"].onToggleChange, function(bool)
	menu_references["hud_armor_color"]:setVisible(bool)
	menu_references["hud_health_color"]:setVisible(bool)
	menu_references["hud_energy_color"]:setVisible(bool)
	menu_references["hud_firearmor_color"]:setVisible(bool)
	hud_styles["Default"]()
	hud_styles[bool and flags["game_hud_style"][1] or "Default"]()
	if bool then
		if not main_gui then
			return end
		...
		bar.HP.bar.BackgroundColor3 = colorfromrgb(36, 182, 3)
		bar.Energy.bar.BackgroundColor3 = colorfromrgb(182, 182, 9)
		bar.Armor.bar.BackgroundColor3 = colorfromrgb(0, 136, 194)
		bar.Armor.firebar.BackgroundColor3 = colorfromrgb(253, 121, 33)
	end
end)
menu_references["hud_armor_color"]:setVisible(false)
menu_references["hud_health_color"]:setVisible(false)
menu_references["hud_energy_color"]:setVisible(false)
menu_references["hud_firearmor_color"]:setVisible(false)
