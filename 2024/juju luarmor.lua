...
skins["Galaxy"] = {
	Color = ColorSequence.new{
		ColorSequenceKeypoint.new(0.00, colorfromrgb(0, 16, 176)),
		ColorSequenceKeypoint.new(1, colorfromrgb(0, 16, 176))        
	},
...
skins["Matrix"] = {
	...
	ShotParticle = newObject("ParticleEmitter", {
	...
	ShotParticle2 = newObject("ParticleEmitter", {
		Color = ColorSequence.new{ColorSequenceKeypoint.new(0,Color3.new(0.411765,0.901961,0.105882)),ColorSequenceKeypoint.new(1,Color3.new(0.411765,0.901961,0.105882))};
		Enabled = false;
		Lifetime = NumberRange.new(0.4,0.4);
		Rate = 1;
...
local purchaseItem = LPH_JIT(function(name)
...
local newNotification = LPH_NO_VIRTUALIZE(function(text, style)
	if style == "Default" then
		...
		local drawing =	utility.newDrawing("Text", {
			...
			Text = text;
			ZIndex = 4;
			Font = tonumber(flags["notification_font"][1])
		})
	...
	for player, info in player_data do
		...
		for _, check in checks do
			...
			elseif check == "Crew" and info["crew"] == lplr_data["crew"] then
				stop = true
				break
			elseif check == "Visible" then
				local raycast_params = RaycastParams.new()
				raycast_params.IgnoreWater = true;
				raycast_params.FilterDescendantsInstances = {character, char, workspace.Ignored, workspace.Vehicles}
				raycast_params.FilterType = Enum.RaycastFilterType.Exclude
				
				if workspace:Raycast(camera_pos, (upperTorso.Position-camera_pos).unit * obj_distance, raycast_params) then
					stop = true
					break
				end
			end
...
local createESPObjects = LPH_NO_VIRTUALIZE(function(player)
	...
	return {
		...
		utility.newDrawing("Text", {
			Center = true,
			Outline = true,
			Size = flags["priority_size"],
			ZIndex = 2,
			Color = flags["priority_flag_color"],
			Transparency = -flags["priority_flag_transparency"]+1,
			Text = "PRIORITY",
			Font = tonumber(flags["esp_font"][1])
		}),
		utility.newDrawing("Text", {
			Center = true,
			Outline = true,
			Size = flags["friendy_size"],
			ZIndex = 2,
			Color = flags["friendly_flag_color"],
			Transparency = -flags["friendly_flag_transparency"]+1,
			Text = "FRIENDLY",
			Font = tonumber(flags["esp_font"][1])
		}),
		utility.newDrawing("Image", {
...
local playerAdded = LPH_NO_VIRTUALIZE(function(player)
	...
	if player.Character then
		task.spawn(playerCharacterAdded, player.Character)
	end
...
local hud_styles = {
	...
	["Minimalistic"] = LPH_JIT(function()
		...
		for _, object in bar:GetChildren() do
			...
			object.TextLabel.Text = string.lower(object.TextLabel.Text)
...
local rage = window:getTab(1)
	local target_section = rage:newSection({name = "Target", is_changeable = true, scale = 1})
		...
		menu_references["target_strafe"] = target_section:newElement({name = "Target strafe", types = {toggle = {flag = "target_strafe"}, keybind = {method = 3, flag = "target_strafe_keybind"}}}); keybinder:add(menu_references["target_strafe"], "Target strafe", "target_strafe_keybind")
		...
		menu_references["view_player"] = target_section:newElement({name = "View player", types = {toggle = {flag = "view_player"}}})
...
local legit = window:getTab(3)
	local aimbot_section = legit:newSection({name = "Aimbot", is_changeable = false, scale = 0.5})
		...
		menu_references["hit_chance"] = aimbot_section:newElement({name = "Hit chance", types = {slider = {flag = "hit_chance", max = 100, default = 100, min = 1, suffix = "%", prefix = ""}}})
		...
		menu_references["cam_lock"] = aimbot_section:newElement({name = "Cam lock", types = {toggle = {flag = "cam_lock"}, keybind = {flag = "cam_lock_keybind"}}}); keybinder:add(menu_references["cam_lock"], "Cam lock", "cam_lock_keybind")
		menu_references["smoothing_style"] = aimbot_section:newElement({name = "Smoothing style", types = {dropdown = {flag = "smoothing_style", no_none = true, default = {"Linear"}, options = {"Linear", "Circular", "Sine", "Quad", "Quint", "Bounce", "Exponential", "Back", "Elastic"}}}})
		menu_references["smoothing_direction"] = aimbot_section:newElement({name = "Smoothing direction", types = {dropdown = {flag = "smoothing_direction", no_none = true, default = {"Out"}, options = {"InOut", "Out", "In"}}}})
		menu_references["camlock_smoothness"] = aimbot_section:newElement({name = "Smoothness", types = {slider = {flag = "camlock_smoothness", min = 1, decimal = 1, max = 100, prefix = "", suffix = "%", changers = 0.1}}})
		menu_references["horizontal_smoothness"] = aimbot_section:newElement({name = "Horizontal smoothness", types = {slider = {flag = "horizontal_smoothness", decimal = 1, min = 1, max = 100, prefix = "", suffix = "%", changers = 0.1}}})
		menu_references["vertical_smoothness"] = aimbot_section:newElement({name = "Vertical smoothness", types = {slider = {flag = "vertical_smoothness", decimal = 1, min = 1, max = 100, prefix = "", suffix = "%", changers = 0.1}}})
		...
		aimbot_section:newElement({name = "Untarget", types = {dropdown = {flag = "untarget", options = {"Target knocked", "Self knocked"}, multi = true}}})
	local gun_settings = legit:newSection({name = "Gun settings", is_changeable = false, x = 0.5, scale = 0.5})
		...
		for gun, _ in gun_types do
			if gun ~= "Default" then
				_["settings"] = {
					...
					gun_settings:newElement({name = "Field of view", types = {slider = {flag = gun.."_field_of_view", min = 5, max = 350, default = 20, suffix = "°", prefix = "", changers = 1}}}),
					gun_settings:newElement({name = "Air hitbox", types = {dropdown = {flag = gun.."_air_hitbox", no_none = true, default = {"LowerTorso"}, options = {"Head", "HumanoidRootPart", "UpperTorso", "LowerTorso", "Closest"}}}}),
					gun_settings:newElement({name = "Hitbox", types = {dropdown = {flag = gun.."_hitbox", no_none = true, default = {"Head"}, options = {"Head", "HumanoidRootPart", "UpperTorso", "LowerTorso", "Closest"}}}})
				}
	...
	local aimbot_settings = legit:newSection({name = "Settings", is_changeable = false, scale = 0.5, y = 0.5})
		...
		menu_references["not_on_vehicles"] = aimbot_settings:newElement({name = "Not on vehicles", types = {toggle = {flag = "not_on_vehicles"}}})
		menu_references["resolver_refresh"] = aimbot_settings:newElement({name = "Refresh rate", types = {slider = {flag = "resolver_refresh", min = 1, default = 18, max = 200, prefix = "", suffix = "ms", changers = 1}}})
	local aimbot_visualization = legit:newSection({name = "Aimbot visualization", is_changeable = false, scale = 0.5, x = 0.5, y = 0.5})
...
local visuals = window:getTab(4)
	local player_esp_section = visuals:newSection({name = "Player ESP", is_changeable = true, scale = 0.6})
		...
		menu_references["friendly_flag"] = player_esp_section:newElement({name = "Friendly flag", types = {toggle = {flag = "friendly_flag"}, colorpicker = {flag = "friendly_flag_color", default = colorfromrgb(0,255,0), transparency_flag = "friendly_flag_transparency"}}})
		menu_references["friendly_size"] = player_esp_section:newElement({name = "Size", types = {slider = {flag = "friendly_size", min = 10, max = 24, default = 14, decimal = 0, prefix = "", suffix = "px"}}})
		menu_references["priority_flag"] = player_esp_section:newElement({name = "Priority flag", types = {toggle = {flag = "priority_flag"}, colorpicker = {flag = "priority_flag_color", default = colorfromrgb(255,0,0), transparency_flag = "priority_flag_transparency"}}})
		menu_references["priority_size"] = player_esp_section:newElement({name = "Size", types = {slider = {flag = "priority_size", min = 10, max = 24, default = 14, decimal = 0, prefix = "", suffix = "px"}}})
	...
	local hud_section = visuals:newSection({name = "Hud", is_changeable = true, x = 0.5, y = 0.3, scale = 0.2})
		...
		menu_references["notifications_"] = hud_section:newElement({name = "Notifications", types = {toggle = {flag = "notifications"}, dropdown = {multi = true, options = {"Target changes", "Hits"}, flag = "notifications_selected"}}})
		menu_references["notification_style"] = hud_section:newElement({name = "Style", types = {dropdown = {options = {"Default", "Minimalistic"}, no_none = true, default = {"Default"}, flag = "notification_style"}}})
...
local misc = window:getTab(5)
	local movement_section = misc:newSection({name = "Movement", is_changeable = false, scale = 0.5})
		...
		menu_references["noclip"] = movement_section:newElement({name = "Noclip", types = {toggle = {flag = "noclip"}, keybind = {flag = "noclip_keybind"}}}); keybinder:add(menu_references["noclip"], "Noclip", "noclip_keybind")
...
local players_tab = window:getTab(7)
	local players_section = players_tab:newSection({name = "Players", is_changeable = true, x = 0, scale = 1})
		...
		menu_references["aim_view"] = players_section:newElement({name = "Aim view", types = {toggle = {no_load = true, flag = "aim_view"}}})
...
local configuration = window:getTab(8)
	local config_section = configuration:newSection({name = "Configs", is_changeable = false, scale = 1})
		...
		config_section:newElement({name = "Save config", types = {button = {text = "Save config", 
			callback = function()
	...
	local other_section = configuration:newSection({name = "Other", is_changeable = false, x = 0.5, y = 1, scale = 0.5})
		utility.newConnection(other_section:newElement({name = "Aim viewer line", types = {colorpicker = {flag = "aim_viewer_line", transparency_flag = "aim_viewer_line_transparency"}}}).onColorChange, function(color, transparency)
...
utility.newConnection(menu_references["cam_lock"].onToggleChange, function(bool)
	menu_references["camlock_smoothness"]:setVisible(bool)
	menu_references["smoothing_style"]:setVisible(bool)
	menu_references["smoothing_direction"]:setVisible(bool)
	menu_references["mouse_aim"]:setVisible(not bool and true or false)
	menu_references["mouse_aim"]:setToggle(false)
end)
menu_references["camlock_smoothness"]:setVisible(false)

utility.newConnection(menu_references["mouse_aim"].onToggleChange, function(bool)
	menu_references["horizontal_smoothness"]:setVisible(bool)
	menu_references["vertical_smoothness"]:setVisible(bool)
	menu_references["smoothing_style"]:setVisible(bool)
	menu_references["smoothing_direction"]:setVisible(bool)

	menu_references["cam_lock"]:setVisible(not bool and true or false)
	menu_references["cam_lock"]:setToggle(false)
	menu_references["camlock_smoothness"]:setVisible(false)
end)
menu_references["horizontal_smoothness"]:setVisible(false)
menu_references["vertical_smoothness"]:setVisible(false)
menu_references["smoothing_style"]:setVisible(false)
menu_references["smoothing_direction"]:setVisible(false)
...
utility.newConnection(menu_references["priority_flag"].onColorChange, function(color, transparency)
	for player, info in player_data do
		local text = info["drawings"][13]
		if text then
			text["Color"] = color
			text["Transparency"] = -transparency+1
		end
	end
end, true)

utility.newConnection(menu_references["priority_flag"].onToggleChange, function(bool)
	menu_references["priority_size"]:setVisible(bool)
end, true)
menu_references["priority_size"]:setVisible(false)

utility.newConnection(menu_references["priority_size"].onSliderChange, function(value)
	for player, info in player_data do
		local text = info["drawings"][13]
		if text then
			text["Size"] = value
...
utility.newConnection(menu_references["friendly_flag"].onColorChange, function(color, transparency)
	for player, info in player_data do
		local text = info["drawings"][14]
		if text then
			text["Color"] = color
			text["Transparency"] = -transparency+1
		end
...
utility.newConnection(menu_references["friendly_flag"].onToggleChange, function(bool)
	menu_references["friendly_size"]:setVisible(bool)
end, true)
menu_references["friendly_size"]:setVisible(false)

utility.newConnection(menu_references["friendly_size"].onSliderChange, function(value)
	for player, info in player_data do
		local text = info["drawings"][14]
		if text then
			text["Size"] = value
...
utility.newConnection(menu_references["esp_font"].onDropdownChange, function(selected)
	...
	for player, info in player_data do
		...
		if drawings then
			...
			local flag = drawings[13]
			local flag2 = drawings[14]
...
utility.newConnection(aimbotTargetChange, LPH_JIT(function(target)
	...
	if flags["notifications"] and find(flags["notifications_selected"], "Target changes") and target ~= old_last_target then
		task.spawn(newNotification, target and "locked onto player "..target.Name or "unlocked", flags["notification_style"][1])
	end
	if lplr_data["position_body"] then
		lplr_data["position_body"]:Destroy()
	end
	if flags["target_strafe"] and flags["target_strafe_keybind"]["active"] and flags["view_player"] then
...
do	
	...
	renderESP = LPH_NO_VIRTUALIZE(function()
		for player, info in player_data do
			...
			if hrp and humanoid then
				...
				local bottom, bottom_visible = camera:WorldToViewportPoint(hrp_pos + below) 
				local top, top_visible = nil, nil

				if not bottom_visible then
					top, top_visible = camera:WorldToViewportPoint(hrp_pos + above)
				end

				if bottom_visible or top_visible then
					if top == nil then 
						top = camera:WorldToViewportPoint(hrp_pos + above)
					end
					...
					if do_priority_flag then
						local text = drawings[13]
						local is_priority = info["aimbot_priority"]

						text.Visible = is_priority

						if text then
							text.Position = vector2new(box_size_x / 2 + box_pos_x, do_name and box_pos_y - text.TextBounds.Y - 2 - drawings[3].TextBounds.Y or box_pos_y - text.TextBounds.Y - 2)
						end
					end

					if do_friendly_flag then
						local text = drawings[14]
						local is_priority = info["whitelisted"]
	...
	utility.newConnection(menu_references["weapon_icon"].onColorChange, LPH_NO_VIRTUALIZE(function(color, transparency)
		if not bool then
			for player, info in player_data do
				...
				if drawings and drawings[15] then
					drawings[15]["Color"] = color
					drawings[15]["Transparency"] = -transparency+1
				end
			end
		end
	end), true)
	...
	utility.newConnection(menu_references["ammo_number"].onToggleChange, LPH_NO_VIRTUALIZE(function(bool)
		...
		if not bool then
			for player, info in player_data do
				...
				if drawings then
					drawings[12]["Visible"] = true
				end
			end
		end
	end), true)

	utility.newConnection(menu_references["priority_flag"].onToggleChange, LPH_NO_VIRTUALIZE(function(bool)
		do_priority_flag = bool
		if not bool then
			for player, info in player_data do
				local drawings = info["drawings"]
	...
	utility.newConnection(menu_references["friendly_flag"].onToggleChange, LPH_NO_VIRTUALIZE(function(bool)
		do_friendly_flag = bool
		if not bool then
			for player, info in player_data do
				local drawings = info["drawings"]
	...
	if (target or flags["auto_select_target"]) and flags["aimbot_keybind"]["active"] then
		...
		if not stop and target then
			...
			if flags["resolver"] then
				if flags["not_on_vehicles"] and character_parts["[CarHitBox]"] then
					velocity = hrp.Velocity
				else
					velocity = aimbot.target_velocity or hrp.Velocity
				end
			end
			...
			if horizontal_prediction == 0 then
				velocity*=vector3new(ping, 1, ping)
			else
				velocity*=vector3new(horizontal_prediction, 1, horizontal_prediction)
			end

			if flags["clamp_y"] and old_velocity.Y < flags["clamp_y_value"] then
				old_velocity*=vector3new(0,flags["clamp_multiplier"],0)
			end

			if vertical_prediction == 0 then
				velocity*=vector3new(1, ping/3, 1)
			else
				velocity*=vector3new(1, vertical_prediction, 1)
			end

			if flags["shake"] then
				local horizontal = flags["horizontal_shake"]
				local vertical = flags["vertical_shake"]
				velocity+=vector3new(mathrandom(1, horizontal+1)/(mathrandom(2) == 1 and -50 or 50), mathrandom(1, vertical+1)/(mathrandom(2) == 1 and -50 or 50), mathrandom(1, horizontal+1)/(mathrandom(2) == 1 and -50 or 50))
			end
			...
			local raycast_params = RaycastParams.new()
			raycast_params.IgnoreWater = true
			raycast_params.FilterDescendantsInstances = {target.Character, char, workspace.Ignored, workspace.Vehicles}
			raycast_params.FilterType = Enum.RaycastFilterType.Exclude

			if check_visibility and workspace:Raycast(hrp_pos, (part_position - hrp_pos).unit * (part_position - hrp_pos).magnitude, raycast_params) then
				aimbot.target_position = nil
				line.Visible = false
				line_outline.Visible = false
				if body and body.Parent == workspace.Ignored then
			...
			if flags["silent_aim"] and flags["max_curve"] then
				local world_position = mouse_position+((new_pos-mouse_position)*(flags["max_curve_value"]/100))
				local ray = camera:ViewportPointToRay(world_position.X, world_position.Y)
			...
			if flags["position_line"] then
				...
				if not origin then
					local _pos, on_screen = camera:WorldToViewportPoint(hrp_pos)
			...
			if flags["look_at"] then
				hrp.CFrame = cframenew(hrp.Position, vector3new(part_position.X, hrp.Position.Y, part_position.Z))
			end
			...
			if flags["mouse_aim"] and flags["mouse_aim_keybind"]["active"] then
				if aimbot.do_tp then
					...
					local direction = Enum.EasingDirection[flags["smoothing_direction"][1]]
					mousemoverel(distance.X * tws:GetValue(((101-flags["horizontal_smoothness"])/100), ease_style, direction), distance.Y * tws:GetValue((101-flags["vertical_smoothness"])/100, ease_style, direction))	
				end
			elseif flags["cam_lock"] and flags["cam_lock_keybind"]["active"] then
				if aimbot.do_tp then
					camera.CFrame = cframenew(camera.CFrame, part_position)
					aimbot.do_tp = false
				else
					camera.CFrame = camera.CFrame:Lerp(cframenew(camera.CFrame.p, part_position), tws:GetValue(((101-flags["camlock_smoothness"])/100), Enum.EasingStyle[flags["smoothing_style"][1]], Enum.EasingDirection[flags["smoothing_direction"][1]]))
				end
			end
		end
	end
...
local renderRPGWarnings = LPH_NO_VIRTUALIZE(function()
	...
	for _, warning in rpg_indicators do
		...
		local distance = warning[2]

		image["Visible"] = false
		distance["Visible"] = false

		local rocket = warning[3]
		local pos = warning[4]
		local visible = warning[5]
		local last_refresh = warning[6]
		...
		if new_tick-last_refresh > 0.175 then
			...
			if result then
				...
				warning[4] = result.Position
				warning[5] = workspace:Raycast(result.Position + lookVector*1, (hrp.Position - result.Position).unit*(hrp.Position - result.Position).magnitude, raycast_params) == nil
				visible = warning[5]
				pos = result.Position
			end
		end
		...
		if hrp then
			...
			if on_screen then
				if visible then
					...
					distance["Text"] = tostring(floor((hrp_pos-pos).magnitude)).." studs"
...
local autoShoot = LPH_NO_VIRTUALIZE(function()
	...
	if workspace:Raycast(pos, (position-pos).unit*(position-pos).magnitude, raycast_params) then
		if is_auto_shooting then
			is_auto_shooting = false
			if tool and gun then
				tool:Deactivate()
...
utility.newConnection(onBulletFired, LPH_JIT(function(object, beam, is_lplr, hit)
	if is_lplr then
		if flags["skin_changer"] then
			local tool = lplr_data["tool"]
			if tool then
				...
				if skin and skin_object then
					...
					if _particle and lplr_data["do_emit"] then
						...
						particle:Emit(skins.ParticleEmit)
					end
				end
			end
		end
...
utility.newConnection(lplrHitPlayer, LPH_JIT(function(player, damage)
	...
	if flags["notifications"] and find(flags["notifications_selected"], "Hits") then
		task.spawn(newNotification, "hit "..player.Name.." for "..damage.. " damage", flags["notification_style"][1])
	end
...
local targetStrafe = LPH_JIT(function(dt)
	...
	lplr_data["strafe_angle"]+=clamp((flags["strafe_speed"]*10)*dt, 0, 360)
...
local autoSort = LPH_JIT(function()
...
utility.newConnection(menu_references["view_player"].onToggleChange, function(bool)
	if not bool and flags["target_strafe"] and aimbot.target then
		camera.CameraSubject = char
	end
end)

utility.newConnection(menu_references["target_strafe"].onActiveChange, function(bool)
	if not bool then
		if flags["view_player"] and aimbot.target then
			camera.CameraSubject = char
		end
	else
		if flags["view_player"] and aimbot.target then
			camera.CameraSubject = aimbot.target.Character
		end
	end
end)
...
utility.newConnection(menu_references["target_strafe"].onToggleChange, function(bool)
	...
	menu_references["view_player"]:setVisible(bool)
...
menu_references["view_player"]:setVisible(false)

utility.newConnection(menu_references["target_strafe"].onActiveChange, function(bool)
	if not bool and flags["view_player"] and aimbot.target then
		camera.CameraSubject = char
	end
end)
